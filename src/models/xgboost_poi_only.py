"""
XGBoost model with POI features but WITHOUT latitude, longitude, and nearby_avg_popularity
Testing if POI features alone can replace location information
"""
import pandas as pd
import numpy as np
...
class XGBoostPOIOnly:
    def __init__(self):
        self.project_root = get_project_root()
        self.data_dir = self.project_root / "data" / "processed" / "daily"
        self.results_dir = self.project_root / "results" / "xgboost_poi_only"
        self.results_dir.mkdir(parents=True, exist_ok=True)
        self.model = None
        
    def load_data(self):
        print("\n" + "="*60)
        print("Loading Data (POI Only, NO lat/lng/popularity)")
        print("="*60)
    ...
    def prepare_features(self, train_df, test_df):
        print("\n" + "="*60)
        print("Feature Engineering (POI Only, NO lat/lng/popularity)")
        print("="*60)
        
        # Base features WITHOUT latitude, longitude, nearby_avg_popularity
        numeric_features = [
    ...
    def train_model(self, X_train, y_train, X_test, y_test):
        print("\n" + "="*60)
        print("Training XGBoost (POI Only, NO lat/lng/popularity)")
        print("="*60)
    ...
    def evaluate_model(self, X_test, y_test, test_df):
        ...
        metrics_file = self.results_dir / "xgboost_metrics_poi_only.csv"
    ...
    def save_results(self, test_df, y_pred, feature_names):
        ...
        predictions_file = self.results_dir / "xgboost_predictions_2024_poi_only.csv"
        predictions_df.to_csv(predictions_file, index=False)
        
        model_file = self.results_dir / "xgboost_model_poi_only.pkl"
        ...
        importance_file = self.results_dir / "feature_importance_poi_only.csv"
...
def main():
    ...
    print("\n" + "="*60)
    print("XGBoost (POI Only, No Location) training complete")
    print("="*60)
